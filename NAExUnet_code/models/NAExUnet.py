...
class NAExU_Net(nn.Module): 
	def __init__(self, args):
		...
		self.decoder_channel = args['decoder_channel']
		self.decoder_concat_level = args['decoder_concat_level']
		self.ext_skip_conv_level = args['ext_skip_conv_level']
		self.dec_reduction_ratio = args['dec_reduction_ratio']
		...
		# level_N		: conv
		#################################################
		for i in range(self.decoder_concat_level, self.n_level):
			...
			if i == 0: setattr(self, 'uconv_{}'.format(i), nn.Conv2d(self.l_channel[-i-1]*3//2+self.l_channel[-i-1]//(self.dec_reduction_ratio[-i-1]*2), self.l_channel[-i-2], kernel_size=1, bias=False))
			elif i == self.n_level-1 : setattr(self, 'uconv_{}'.format(i), nn.Conv2d(self.l_channel[-i-1]*3//2+self.l_channel[-i-1]//(self.dec_reduction_ratio[-i-1]*2), self.l_channel[0], kernel_size=1, bias=False))
			else: setattr(self, 'uconv_{}'.format(i), nn.ConvTranspose2d(self.l_channel[-i-1]*3//2+self.l_channel[-i-1]//(self.dec_reduction_ratio[-i-1]*2), self.l_channel[-i-2], kernel_size=2, stride=2, bias=False))
	...
	def forward(self, x, only_code=False):
		...
		for i in range(1, self.n_level+1):
			if self.n_level-i < self.decoder_concat_level:
				uc_dx['%d'%(self.n_level-i)] = torch.cat((uc_sv_dx['%d'%(self.n_level-i)], uc_noise_dx['%d'%(self.n_level-i)]), 1)
