...
class ExU_Net2(nn.Module): 
	def __init__(self, args):
		super(ExU_Net2, self).__init__()
		self.args = args
		...
		self.fc = nn.Linear(final_dim*4, self.code_dim)
	...
	def forward(self, x, only_code=False):
		...
		encoder_final_feature = d_dx['%d'%(self.n_level)]

		bs, _, _, time = encoder_final_feature.size()
		x = encoder_final_feature.reshape(bs, -1, time)

		w = self.attention_encoder(x)
		m = torch.sum(x * w, dim=-1)
		s = torch.sqrt((torch.sum((x ** 2) * w, dim=-1) - m ** 2).clamp(min=1e-5))
		x = torch.cat([m, s], dim=1)
		encoder_x = self.bn_agg_encoder(x)
